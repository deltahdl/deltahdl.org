...
class TestCancelMain:
    ...
    def test_no_running_workflows_exits_zero(self, cancel, temp_graph_file) -> None:
    ...
    def test_invalid_running_json_exits_one(self, cancel, temp_graph_file) -> None:
    ...
    def test_missing_graph_exits_one(self, cancel) -> None:
        ...
        test_args = [
            "cancel", "--repo", "owner/repo",
            "--changed-files", "src/bootstrap/main.py",
            "--running", '["www_redirect"]', "--graph", "/nonexistent/graph.json",
        ]
        with patch.object(sys, "argv", test_args):
            assert cancel.main() == 1

    @patch("cancel.get_cancelable_runs")
    def test_no_matching_runs_exits_zero(self, mock_get_runs, cancel, temp_graph_file) -> None:
        """Main returns 0 when no runs match workflows to cancel."""
        mock_get_runs.return_value = []
        test_args = [
            "cancel", "--repo", "owner/repo",
            "--changed-files", "src/bootstrap/main.py",
            "--running", '["www_redirect"]', "--graph", temp_graph_file,
        ]
        with patch.object(sys, "argv", test_args):
            assert cancel.main() == 0
    ...
    def test_successful_cancel_exits_zero(
        ...
        mock_get_runs.side_effect = [[{"id": 123, "name": "WWW Redirect", "run_number": 1}], []]
...
class TestDispatchRootsMain:
    ...
    def test_invalid_running_json_exits_one(self, dispatch_roots, temp_graph_file) -> None:
        """Main returns 1 for invalid --running JSON."""
        test_args = [
            "dispatch_roots", "--repo", "owner/repo",
            "--changed-files", "src/bootstrap/main.py",
    ...
    def test_no_roots_exits_zero(self, dispatch_roots, temp_graph_file) -> None:
...
class TestGetRunningMain:
    ...
    def test_returns_running_workflow_keys(
        ...
        mock_get_runs.side_effect = [[{"name": "WWW Redirect"}, {"name": "Bootstrap"}], []]
        output = self._run_and_parse(get_running, temp_graph_file, capsys)
...
class TestComputeRootsMain:
    ...
    def test_outputs_root_workflow(self, compute_roots, temp_graph_file, capsys) -> None:
        """Main outputs root workflow for changed files."""
        args = ["--changed-files", "src/bootstrap/main.py", "--graph", temp_graph_file]
        output = self._run_and_parse(compute_roots, args, capsys)
        assert output == {"workflows": ["bootstrap"]}

    def test_start_from_overrides_file_detection(self, compute_roots, temp_graph_file, capsys) -> None:
        """Main uses --start-from instead of file detection."""
        args = [
            "--changed-files", "src/bootstrap/main.py",
            "--start-from", "www_redirect", "--graph", temp_graph_file,
        ]
        output = self._run_and_parse(compute_roots, args, capsys)
    ...
    def test_levels_first_level_has_bootstrap(self, compute_roots, temp_graph_file, capsys) -> None:
        """Main with --levels has bootstrap in first level."""
        args = ["--changed-files", "src/bootstrap/main.py", "--levels", "--graph", temp_graph_file]
        output = self._run_and_parse(compute_roots, args, capsys)
        assert "bootstrap" in output["levels"][0]

    def test_indexed_output_structure(self, compute_roots, temp_graph_file, capsys) -> None:
    ...
    def test_indexed_workflow_has_required_fields(self, compute_roots, temp_graph_file, capsys) -> None:
    ...
    def test_slots_output_has_count_and_keys(self, compute_roots, temp_graph_file, capsys) -> None:
    ...
    def test_running_merges_with_roots(self, compute_roots, temp_graph_file, capsys) -> None:
